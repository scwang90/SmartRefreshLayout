    ...
    class FlingRunnable implements Runnable {
        ...
        public Runnable start() {
            if (mSpinner != 0 && (!(mState.opening)
                    || ((mState == RefreshState.Loading || mFooterNoMoreData) && mSpinner < -mFooterHeight)
...
//                    }
    ...
    protected void moveSpinnerInfinitely(float spinner) {
        ...
        if (mEnableAutoLoadMore && !mFooterNoMoreData
    ...
    public void onNestedScroll(@NonNull View target, int dxConsumed, int dyConsumed, int dxUnconsumed, int dyUnconsumed) {
        ...
        if (mState.opening) {
            ...
            if (isEnableRefresh() && dy < 0 && (mRefreshContent == null || mRefreshContent.canRefresh())) {
                ...
                if (mState == RefreshState.None && !mFooterNoMoreData) {
                    mKernel.setState(RefreshState.PullUpToLoad);
                }
                mTotalUnconsumed -= Math.abs(dy);
                moveSpinnerInfinitely(mTotalUnconsumed);
     ...
     */
    @Override
    public SmartRefreshLayout setEnableClipHeaderWhenFixedBehind(boolean enable) {
        this.mEnableClipHeaderWhenFixedBehind = enable;
        return this;
    }
     ...
     */
    @Override
    public SmartRefreshLayout setEnableClipFooterWhenFixedBehind(boolean enable) {
        this.mEnableClipFooterWhenFixedBehind = enable;
        return this;
    }
     ...
     */
    @Override
    public SmartRefreshLayout setPrimaryColors(@ColorInt int... colors) {
        if (mRefreshHeader != null) {
        ...
        public RefreshKernel setState(@NonNull RefreshState state) {
            switch (state) {
                ...
                case PullDownToRefresh:
                    ...
                    break;
                case PullUpToLoad:
                    if (isEnableLoadMore() && !mFooterNoMoreData && !mState.opening && !mState.finishing) {
                ...
                case PullDownCanceled:
                    ...
                    break;
                case PullUpCanceled:
                    if (isEnableLoadMore() && !mFooterNoMoreData && !mState.opening) {
